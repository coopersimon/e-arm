...
pub fn decode_instruction(i: u32) -> ARMv4Instruction {
    ...
    let instr = match i & bits(26, 27) {
        COPROC      => decode_coproc(i),
        BRANCH      => decode_branch(i),
        TRANSFER    => decode_transfer(i),
        ALU         => decode_alu(i),
        _ => unreachable!(),
    };
...
fn decode_coproc(i: u32) -> ARMv4InstructionType {
    if test_bit(i, 25) {
        if test_bit(i, 24) {
        ...
        decode_coproc_transfer(i)
    }
}

/// Decode a coprocessor register transfer or data op.
fn decode_coproc_op(i: u32) -> ARMv4InstructionType {
...
fn decode_coproc_transfer(i: u32) -> ARMv4InstructionType {
...
fn decode_branch(i: u32) -> ARMv4InstructionType {
    if test_bit(i, 25) {
...
fn decode_transfer(i: u32) -> ARMv4InstructionType {
    let base_reg = ((i >> 16) & 0xF) as usize;
...
fn decode_alu(i: u32) -> ARMv4InstructionType {
...
fn decode_other(i: u32) -> ARMv4InstructionType {
    if (i & bits(5, 6)) == 0 {
        if test_bit(i, 24) {
            decode_swp(i)
        } else {
...
fn decode_data_proc(i: u32) -> ARMv4InstructionType {
    let set_flags = test_bit(i, 20);
    let rn = ((i >> 16) & 0xF) as usize;
    let rd = ((i >> 12) & 0xF) as usize;
    ...
    match (i >> 21) & 0xF {
        0x0 => ARMv4InstructionType::AND{rd, rn, op2: decode_alu_op2(i), set_flags},
        0x1 => ARMv4InstructionType::EOR{rd, rn, op2: decode_alu_op2(i), set_flags},
        0x2 => ARMv4InstructionType::SUB{rd, rn, op2: decode_alu_op2(i), set_flags},
        0x3 => ARMv4InstructionType::RSB{rd, rn, op2: decode_alu_op2(i), set_flags},
        0x4 => ARMv4InstructionType::ADD{rd, rn, op2: decode_alu_op2(i), set_flags},
        0x5 => ARMv4InstructionType::ADC{rd, rn, op2: decode_alu_op2(i), set_flags},
        0x6 => ARMv4InstructionType::SBC{rd, rn, op2: decode_alu_op2(i), set_flags},
        0x7 => ARMv4InstructionType::RSC{rd, rn, op2: decode_alu_op2(i), set_flags},
        0xC => ARMv4InstructionType::ORR{rd, rn, op2: decode_alu_op2(i), set_flags},
        0xE => ARMv4InstructionType::BIC{rd, rn, op2: decode_alu_op2(i), set_flags},
        0x8 => if compare() {
            ARMv4InstructionType::TST{rn, op2: decode_alu_op2(i)}
        } else {
            decode_ext_mode(i, rn, rd, false, false)
        },
        0x9 => if compare() {
            ARMv4InstructionType::TEQ{rn, op2: decode_alu_op2(i)}
        } else {
            decode_ext_mode(i, rn, rd, true, false)
        },
        0xA => if compare() {
            ARMv4InstructionType::CMP{rn, op2: decode_alu_op2(i)}
        } else {
            decode_ext_mode(i, rn, rd, false, true)
        },
        0xB => if compare() {
            ARMv4InstructionType::CMN{rn, op2: decode_alu_op2(i)}
        } else {
            decode_ext_mode(i, rn, rd, true, true)
        },
        0xD => ARMv4InstructionType::MOV{rd, op2: decode_alu_op2(i), set_flags},
        0xF => ARMv4InstructionType::MVN{rd, op2: decode_alu_op2(i), set_flags},
        _ => ARMv4InstructionType::UND,
    }
}
...
fn decode_transfer_halfword(i: u32) -> ARMv4InstructionType {
    let base_reg = ((i >> 16) & 0xF) as usize;
    let data_reg = ((i >> 12) & 0xF) as usize;
